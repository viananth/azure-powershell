...
function Get-Product
{
    ...
    if ('Products_List' -eq $PsCmdlet.ParameterSetName) {
        ...
        $taskResult = $AzureBridgeAdminClient.Products.ListWithHttpMessagesAsync()
    } elseif ('Products_Get' -eq $PsCmdlet.ParameterSetName ) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $AzureBridgeAdminClient.'
        $taskResult = $AzureBridgeAdminClient.Products.GetWithHttpMessagesAsync($ProductName)
    } else {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.AzureStack.Management.AzureBridge.Admin.Models.ProductResource]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $AzureBridgeAdminClient.Products.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
