...
function Suspend-AzsInfrastructureRoleInstance {
    ...
    Process {
        ...
        if ($PSCmdlet.ShouldProcess("$Name" , "Disable infrastructure role instance")) {
            if (-not ($Force.IsPresent -or $PSCmdlet.ShouldContinue("Disable infrastructure role instance?", "Performing operation disable infrastructure role instance for $Name"))) {
                return;
            }
        }
