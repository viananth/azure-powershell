...
function Get-Activation
{
    ...
    } elseif ('Activations_Get' -eq $PsCmdlet.ParameterSetName ) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $AzureBridgeAdminClient.'
        $taskResult = $AzureBridgeAdminClient.Activations.GetWithHttpMessagesAsync($ActivationName)
    } else {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.AzureStack.Management.AzureBridge.Admin.Models.ActivationResource]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $AzureBridgeAdminClient.Activations.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
