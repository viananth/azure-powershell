...
{
    [OutputType([Microsoft.AzureStack.Management.AzureBridge.Admin.Models.RegistrationResource])]
    [CmdletBinding(DefaultParameterSetName='Registrations_List')]
    param(    
        [Parameter(Mandatory = $false, ParameterSetName = 'Registrations_List')]
        [int]
        $Skip = -1,
    ...
    if ('Registrations_List' -eq $PsCmdlet.ParameterSetName) {
        ...
        $taskResult = $AzureBridgeAdminClient.Registrations.ListWithHttpMessagesAsync()
        ...
        $taskResult = $AzureBridgeAdminClient.Registrations.GetWithHttpMessagesAsync($RegistrationName)
    } else {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.AzureStack.Management.AzureBridge.Admin.Models.RegistrationResource]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $AzureBridgeAdminClient.Registrations.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
