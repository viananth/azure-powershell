...
function Get-DownloadedProduct
{
    ...
    if ('DownloadedProducts_List' -eq $PsCmdlet.ParameterSetName) {
        ...
        $taskResult = $AzureBridgeAdminClient.DownloadedProducts.ListWithHttpMessagesAsync($ActivationName)
    } elseif ('DownloadedProducts_Get' -eq $PsCmdlet.ParameterSetName ) {
        Write-Verbose -Message 'Performing operation GetWithHttpMessagesAsync on $AzureBridgeAdminClient.'
        $taskResult = $AzureBridgeAdminClient.DownloadedProducts.GetWithHttpMessagesAsync($ActivationName, $ProductName)
    } else {
    ...
    if ($TaskResult) {
           ...
           $taskResult.Result.Body)
        {
            ...
            if ($result -is [Microsoft.Rest.Azure.IPage[Microsoft.AzureStack.Management.AzureBridge.Admin.Models.DownloadedProductResource]]) {
        ...
        while ($result -and (Get-Member -InputObject $result -Name 'nextLink') -and $result.'nextLink' -and (($Top -eq -1) -or ($returnedCount -lt $Top))) {
            ...
            $taskResult = $AzureBridgeAdminClient.DownloadedProducts.ListNextWithHttpMessagesAsync($result.'nextLink')
             $result = $null
        $ErrorActionPreference = 'Stop'
